...
# Number of taxa is 226 in current ps vs 259 in original ps
...
#anova (WITHOUT random effect of Fecal.ID)
CPTf23_aov_SR <- aov(SR ~ Population, data = df_CPTf23)
CPTf23_aov_SR
summary(CPTf23_aov_SR) # Location is still significant p =0.000661 vs OLD:p = 0.0000398
plot(CPTf23_aov_SR) 
# Above plot gives 5 different plots:
# 1. Residuals vs Fitted (want the red line to be relatively straight) [it is]
# 2. Normal Q-Q (want them to be linear) [are mostly..]
# 3. Scale-Location (not really sure what this is or what we want it to look like? flat?)
# 4. Residuals vs. Leverage (Cook's distance) - checks for outliers? Want samples to be within the dashed line [i think this looks good?]
### 

# check distribution of SR
hist(df_CPTf23$SR) # distribution looks... skewed-ish?

# Use Shapiro-Wilk normality test (for SR WITHOUT random effects)
...
CPTf_aov_PD
summary(CPTf_aov_PD) # Population p = 8.43e-05 vs OLD pval = 0.00049 (significant)
plot(CPTf_aov_PD) 
# Above plot gives 5 different plots:
# 1. Residuals vs Fitted (want the red line to be relatively straight)
# 2. Normal Q-Q (want them to be linear)
# 3. Scale-Location (not really sure what this is or what we want it to look like? flat?)
# 4. Residuals vs. Leverage (Cook's distance) - checks for outliers? Want samples to be within the dashed line
...
# Use Shapiro-Wilk normality test (for PD WITHOUT random effects)
shapiro.test(df_CPTf23$PD) # want pval to be > 0.05 to pass test; it is JUST barely p = 0.05412
...
# Box plot of PD by Sample Day with lines connecting mean values
CPTf.PD <- df_CPTf23%>% 
...
# Create distance measurements and matrix
unweigh_CPTf23 <- phyloseq::distance(CPTf23, "unifrac") # 225 ASVs in CPTf23 but 449 in the tree. Means one of the ASVs that was used to make the tree is one of the base roots. Need to remake tree from sequences AFTER decontam. DONE 11.24.23 (works now without error!)
...
```{r, eval=FALSE}
library(MicEco)
# Difference in ps_euler vs. ps_venn: euler changes the size of the circles proportional to the counts/percentages within them. Example: ccf has more ASVs so the CCF circle is bigger than SCBI. regular ps_venn does NOT change circle size, only lists numbers.
ps_euler(
  CPTf23,
  "Population",
  fraction = 0,
...
# LefSe is better at dealing with all the zeroes of microbiome data
...
#View(cptf23_lefse_mt)
#CPTf23_mt_df <- as.data.frame(cptf23_lefse_mt)
# write out marker table to csv
#write.csv(cptf23_lefse_mt, file = "/Users/morganmaly/OneDrive - Smithsonian Institution/CheetahGutMicrobiome/COMBINED_PoopTrials/CPTf_2023_fresh_data/CPTf23_LefSe/CPTf23_LefSe_LDA4_markertable.csv")

sorted_cptf23_lefse_mt <- cptf23_lefse_mt[order(cptf23_lefse_mt$padj, decreasing = FALSE), ]
head(sorted_cptf23_lefse_mt)
#View(sorted_cptf23_lefse_mt)
...
# See "CPTf23_ALDEx2_PICRUSt2.Rmd" for statistical tests of Picrust2 categories and levels
# See "CPTf23_ALDEx2_Picrust2_Visualizations.Rmd" for creating figures from ALDEx2 anaylses of Picrust2 data
```